            ...
            <section class="wrapper">
                ...
                <div class="top-page-header">
                    ...
                    <div class="page-title">
                        <h2>Left Advertisers</h2>
                    </div>
                    <div class="page-breadcrumb">
                        <nav class="c_breadcrumbs">
                            <ul>
                                <li><a href="">Dashboard</a></li>
                                <li class="active"><a href="">Left Advertisers</a></li>
                            </ul>
                        </nav>
                    </div>
...
//count records
if($_POST['email']){
	$record_count = mysqli_num_rows(mysqli_query($con, "SELECT * FROM users WHERE status='Inactive' AND email='".$_POST['email']."' ORDER BY 1 DESC "));
}else{
	$record_count = mysqli_num_rows(mysqli_query($con, "SELECT * FROM users WHERE status='Inactive' ORDER BY 1 DESC "));
}
...
if($_POST['email']){
	$get = mysqli_query($con,"SELECT * FROM users  WHERE status='Inactive' AND email='".$_POST['email']."' ORDER BY 1 DESC  LIMIT $start, $per_page");
}else{
	$get = mysqli_query($con,"SELECT * FROM users  WHERE status='Inactive' ORDER BY 1 DESC  LIMIT $start, $per_page");
}
...
if($_POST['email']){
	$GetAdvertisers	= mysqli_query($con,"SELECT * FROM users WHERE status='Inactive' AND email='".$_POST['email']."' ORDER BY 1 DESC  LIMIT $start, $per_page");

}else{
	$GetAdvertisers	= mysqli_query($con,"SELECT * FROM users WHERE status='Inactive' ORDER BY 1 DESC  LIMIT $start, $per_page");
}
	$sno=0;
...
if(isset($_POST['change_status'])){
	...
	//$updateStatusAds	= mysqli_query($con,"UPDATE post_ad SET status='Unblocked' WHERE email='".$_POST['getemail']."'");
	$updateStatus		= mysqli_query($con,"UPDATE users SET status='".$_POST['status']."', reason='' WHERE id='".$_POST['getid']."'");
	if($updateStatus){
		echo '<script>window.location.href = "index.php?page=advertisers";</script>';
	}
}
?>				
					</tr>
